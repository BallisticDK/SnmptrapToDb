...
these script are for parsing snmp trap events.

Running it

listenfortraps.sh | php trapParser.php

this comamnd pipes the data from snmptrapd to the php script wich parses it and spits it into an array for use later.

right now it takes the array and parses it further so we can stick it into a database for use in a C# program
...
in the to add a new trap it should parse and put into the database, append this,
